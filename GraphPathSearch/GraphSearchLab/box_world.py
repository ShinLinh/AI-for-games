...
class BoxWorld(object):
    ...
    def __init__(self, nx, ny, cx, cy):
        ...
        self.target = None
        self.agent = None
        # The waypoint index of the current path
        self.current_index = -1
        
        #List containing the boxes that hold items
        self.item_boxes = []
    ...
    def update(self, delta):
        ...
        if self.agent.is_moving == True and self.agent.arrived() == False:
            self.agent.update(delta)
        # Update the waypoint for the agent
        if self.path:
            #Increment the waypoint index 
            if self.agent.arrived():
                if self.current_index < len(self.path.path): 
                    self.agent.update_waypoint(self.path.path[self.current_index])
                    self.current_index += 1
    ...
    def set_item(self, idx):
        '''Set the boxes carrying items based on its index values in idxs. '''
        
        for box in self.item_boxes:
            if box.idx == idx:
                box.marker = None
                self.item_boxes.remove(box)

        if self.start == self.boxes[idx]:
            print("Can't have the same box as the start box!")
            return
        if self.target == self.boxes[idx]:
            print("Can't have the same box as the end box!")
            return

        item_box = self.boxes[idx]
        item_box.marker = 'i'
        self.item_boxes.append(item_box)
            

    def plan_path(self, search, search_type, limit):
        ...
        specified in `search`.
        '''
        # Get the closest waypoint to use as the starting point for the path
        shortest_distance = self.agent.distance_to(self.boxes[0]._vc)
        start_point_index = self.boxes[0].idx
        for i in range(len(self.boxes)):
            if self.agent.distance_to(self.boxes[i]._vc) < shortest_distance:
                shortest_distance = self.agent.distance_to(self.boxes[i]._vc)
                start_point_index = self.boxes[i].idx
        # Reset the path index when planning a new path
        self.current_index = 0
        # Update the next waypoint for the agent
        self.agent.update_waypoint(start_point_index)

        # When the search type is search-to-point, the AI finds a path to the destination(target) using the chosen search method
        if search_type == 'position':
            cls = SEARCHES[search]
            self.path = cls(self.graph, start_point_index, self.target.idx, limit)
            self.current_index = 0
        else:
            # When the search type is search-for-item
            #Check if there is any item in the box world
            if len(self.item_boxes) > 0:
                possible_paths = []
                # Find a path to each of the item in the box world
                for box in self.item_boxes:
                    cls = SEARCHES[search]
                    possible_paths.append(cls(self.graph, start_point_index, box.idx, limit))
                # Find the item with the lowest cost path
                shortest_item_path_cost = self.graph.path_cost(possible_paths[0].path)
                closest_item = possible_paths[0]
                for path in possible_paths:
                    # Add the cost to get to each item to the item marker so that it can be displayed during search-to-point mode
                    self.boxes[path.target_idx].marker = 'i:' + str(round(self.graph.path_cost(path.path), 2))
                    if self.graph.path_cost(path.path) < shortest_item_path_cost:
                        shortest_item_path_cost = self.graph.path_cost(path.path)
                        closest_item = path
                cls = SEARCHES[search]
                self.path = closest_item
