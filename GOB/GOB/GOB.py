...
actions = {
    'get raw food': { 'Eat': -3, 'Toilet': 2, 'Sickness': 4, 'Sleep': 3},
    'get snack': { 'Eat': -2, 'Toilet': 1, 'Sickness': 3, 'Sleep': 2},
    'sleep in bed': { 'Sleep': -4, 'Sickness': -4, 'Eat': 3 },
    'sleep on sofa': { 'Sleep': -2, 'Sickness': -2, 'Eat': 2 },
    'use toilet': {'Toilet': -3, 'Eat': 3, 'Sleep': 2}
}
...
def apply_action(action):
    ...
    for discontentment, change in list(actions[action].items()):
        valid_goal = True
        try:
            discontentments[discontentment]
        except KeyError:
            valid_goal = False
        if valid_goal:
            discontentments[discontentment] = discontentments[discontentment] + change

        for goal in list(goals):
            if goals[goal] < 0:
                goals[goal] = 0
        for discontentment in list(discontentments):
            if discontentments[discontentment] < 0:
                discontentments[discontentment] = 0
...
def choose_action():
    ...
    assert len(discontentments) > 0, 'Need at least one discontentment'

    # Find the most insistent goal - the 'Pythonic' way...
    best_goal, best_goal_value = max(list(goals.items()), key=lambda item: item[1])
    best_discontentment, best_discontentment_value = max(list(discontentments.items()), key=lambda item: item[1])
    #print(best_discontentment_value)
    if best_discontentment_value > best_goal_value:
        best_goal = best_discontentment
        best_goal_value = best_discontentment_value
    ...
    if VERBOSE: 
        try: 
            print('BEST_GOAL:', best_goal, goals[best_goal])
        except KeyError:
            print('BEST_GOAL:', best_goal, discontentments[best_goal])
...
def run_until_all_goals_zero():
    ...
    while running:
        sleep(1.0)
        print('GOALS:', goals)
        print('DISCONTENTMENT:', discontentments)
        ...
        print('NEW DISCONTENTMENT:', discontentments)
        # Stop?
        if all(value == 0 for goal, value in list(goals.items())):
            running = False
        for discontentment in list(discontentments):
            if discontentments[discontentment] > 10:
                running = False
        print(HR)
