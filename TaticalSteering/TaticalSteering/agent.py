...
class Agent(object):
    ...
    def update(self, delta):
        ...
        force = self.calculate(delta) + self.avoid_obstacles_force()
    ...
    def render(self, color=None):
        ...
        if self.mode == 'follow_path':
            self.path.render()
        if self.mode == 'wander' or self.mode == 'hunt':
...
## OPTIONAL EXTRA... pursuit (you'll need something to pursue!)
    ...
    def avoid_obstacles_force(self):
        ...
        for obstacle in self.world.obstacles:
            local_position = self.world.transform_point(obstacle.pos, self.pos, self.heading, self.side)
